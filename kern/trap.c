...
page_fault_handler(struct Trapframe *tf)
{
	...
	// LAB 3: Your code here.
	if ((tf->tf_cs & 3) == 0) {
		panic("page_fault_handler: page fault in kernel mode, fault address %d.\n", fault_va);
	}
	...
	if (curenv->env_pgfault_upcall) {
		...
		if (tf->tf_esp >= UXSTACKTOP - PGSIZE && tf->tf_esp < UXSTACKTOP) {
			stacktop = tf->tf_esp - utf_size - 4;
		}
		else {
			stacktop = UXSTACKTOP - utf_size;
		}

		user_mem_assert(curenv, (void *)stacktop, utf_size, PTE_U|PTE_W);
